...
### Season: {seasonalContext} ";
        ...
        public async Task<(UserAccommodationPreferences, UserFoodPreferences, UserPersonalization)> GetUserPreferencesFromMessage(string message)
        {
            ...
            var responseText = await _apiClient.GenerateContentAsync(prompt, expectJson: true);

            try
            {
                var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
                var preferences = JsonSerializer.Deserialize<PreferencesResponse>(responseText, options);
            ...
            catch (JsonException exception)
            {
                throw new GeminiAPIResponseParseException(
                    $"Could not parse the preferences JSON. Response: {responseText}", exception);
            }
        }
